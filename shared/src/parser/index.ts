...
import { AST, ASTNode, NumberNode, IdentifierNode, PriceDataNode, BinaryOperationNode, UnaryOperationNode, FunctionCallNode, ConditionalNode } from './ast';
 ...
 * @returns Parsed AST
 */
export function parse(expression: string): AST {
...
class TechnicalIndicators {
  ...
  static rsi(values: number[], period: number): number[] {
    const result: number[] = [];
    const gains: number[] = [];
    const losses: number[] = [];
  ...
  static crossover(series1: number, series2: number, currentIndex: number): boolean {
  ...
  static crossunder(series1: number, series2: number, currentIndex: number): boolean {
    // For simplified implementation, we just check if series1 < series2
    // In a full implementation, we'd need historical values to check crossing
    return series1 < series2;
  }
 ...
 */
export function executeNode(ast: ASTNode, context: ExecutionContext): any {
  switch (ast.type) {
    case 'Number':
    ...
    case 'Identifier':
      const identifier = (ast as IdentifierNode).name;
    ...
    case 'PriceData':
      const priceField = (ast as PriceDataNode).field;
    ...
    case 'UnaryOperation':
      const unaryOp = ast as UnaryOperationNode;
    ...
    case 'FunctionCall':
      ...
      switch (funcCall.name) {
        // Traditional names
        case 'sma':
          if (args.length !== 2) throw new Error('sma requires 2 arguments: source, period');
          const smaSource = context.candles.map(c => c.close);
          return TechnicalIndicators.sma(smaSource, args[1])[context.currentIndex];
          
        case 'ema':
          if (args.length !== 2) throw new Error('ema requires 2 arguments: source, period');
          const emaSource = context.candles.map(c => c.close);
          return TechnicalIndicators.ema(emaSource, args[1])[context.currentIndex];
          
        case 'rsi':
          if (args.length !== 2) throw new Error('rsi requires 2 arguments: source, period');
          const rsiSource = context.candles.map(c => c.close);
          return TechnicalIndicators.rsi(rsiSource, args[1])[context.currentIndex];
          
        case 'atr':
          if (args.length !== 1) throw new Error('atr requires 1 argument: period');
          return TechnicalIndicators.atr(context.candles, args[0])[context.currentIndex];
          
        // Pine Script ta.* functions
        case 'ta.sma':
          if (args.length !== 2) throw new Error('ta.sma requires 2 arguments: source, period');
          const taSmaSource = context.candles.map(c => c.close);
          return TechnicalIndicators.sma(taSmaSource, args[1])[context.currentIndex];
          
        case 'ta.ema':
          if (args.length !== 2) throw new Error('ta.ema requires 2 arguments: source, period');
          const taEmaSource = context.candles.map(c => c.close);
          return TechnicalIndicators.ema(taEmaSource, args[1])[context.currentIndex];
          
        case 'ta.rsi':
          if (args.length !== 2) throw new Error('ta.rsi requires 2 arguments: source, period');
          const taRsiSource = context.candles.map(c => c.close);
          return TechnicalIndicators.rsi(taRsiSource, args[1])[context.currentIndex];
          
        case 'ta.atr':
          if (args.length !== 1) throw new Error('ta.atr requires 1 argument: period');
          return TechnicalIndicators.atr(context.candles, args[0])[context.currentIndex];
          
        case 'ta.crossover':
          if (args.length !== 2) throw new Error('ta.crossover requires 2 arguments: series1, series2');
          return TechnicalIndicators.crossover(args[0], args[1], context.currentIndex);
          
        case 'ta.crossunder':
          if (args.length !== 2) throw new Error('ta.crossunder requires 2 arguments: series1, series2');
          return TechnicalIndicators.crossunder(args[0], args[1], context.currentIndex);
          
        case 'math.abs':
          if (args.length !== 1) throw new Error('math.abs requires 1 argument: value');
          return Math.abs(args[0]);
          
        case 'math.max':
          if (args.length !== 2) throw new Error('math.max requires 2 arguments: value1, value2');
          return Math.max(args[0], args[1]);
          
        case 'math.min':
          if (args.length !== 2) throw new Error('math.min requires 2 arguments: value1, value2');
 ...
 * @returns The result of execution
 */
export function execute(ast: AST, candles: Candle[], variables: Map<string, any> = new Map()): any {
 ...
 */
export function executeBacktest(ast: AST, candles: Candle[], variables: Map<string, any> = new Map()): any[] {
  const results: any[] = [];
